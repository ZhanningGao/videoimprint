...
classdef LookUpTableVec < Module
    properties
        sz;
        out_dim;
        weight;
    ...
    methods
        function obj = LookUpTableVec(sz, output_dim)
            obj = obj@Module();
            obj.sz = sz;
        ...
        function output = fprop(obj, input)
            ...
            obj.output = obj.weight.D * reshape(input,[size(input,1),size(input,2)*size(input,3)]);
            obj.output = squeeze(reshape(obj.output, [obj.out_dim, size(input,2), size(input,3)]));
            output = obj.output;
        end
