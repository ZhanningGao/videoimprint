function [pi,pl,Lq,loglikelihood_samples,loglikelihood,H] = tcgH( counts, E, W, S, options )
...
if options.normalize_data
      for s = 1:S*S
        ...
        %  counts((s-1)*Z+1:s*Z,:) = 100*prod(W*S)*bsxfun( @rdivide, tmp_counts, sum(tmp_counts,1) );
          counts((s-1)*Z+1:s*Z,:) = 100*prod(W*S)*(yael_vecs_normalize(tmp_counts).^2);
      end
...
while ~converged
    ...
    if options.plot_figure == 1;
        ...
        figure(2);imagesc(sum(Lq,3));drawnow;
        %pause(0.2);
    end
